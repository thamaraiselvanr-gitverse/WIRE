...
from wire.schema.layout_schema import RemovalPlan
...
class LayoutReflowEngine:
    ...
    def execute(cids_root: ComponentNode, plan: RemovalPlan) -> ComponentNode:
        ...
        section_lookup = SectionRemovalPlanner.find_node_by_path(
            mutated_root, plan.section_node_path
        )
        if not section_lookup:
            raise ValueError(
                f"Section node not found in mutated tree: {plan.section_node_path}"
            )
        ...
        for action in plan.reflow_actions:
            target_lookup = SectionRemovalPlanner.find_node_by_path(
                mutated_root, action.target_node_path
            )
            if target_lookup:
                target_node, target_parent, target_idx = target_lookup
                resolved_actions.append(
                    (action, target_node, target_parent, target_idx)
                )
            else:
                logger.warning(
                    "reflow_target_not_resolved", path=action.target_node_path
                )
        ...
        for parent_node, child_node in nodes_to_remove:
            parent_node.children = [
                c for c in parent_node.children if c is not child_node
            ]
            logger.info("reflow_nav_entry_removed", tag=child_node.tag)

        # 2. Perform structural removal of the section itself
        if section_parent:
            section_parent.children = [
                c for c in section_parent.children if c is not section_node
            ]
