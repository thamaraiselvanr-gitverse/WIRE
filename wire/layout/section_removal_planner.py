...
class SectionRemovalPlanner:
    ...
    @staticmethod
    def find_node_by_path(root: ComponentNode, path: str) -> Optional[Tuple[ComponentNode, Optional[ComponentNode], int]]:
        """
    ...
    @staticmethod
    def find_dependent_nav_links(node: ComponentNode, section_id: str, current_path: str = "root") -> List[str]:
        """Recursively find all anchor links pointing to the section ID."""
        links = []
        href = node.attributes.get("href", "").strip()
        if node.tag == "a" and (href == f"#{section_id}" or href.startswith(f"#{section_id}/")):
            links.append(current_path)

        for idx, child in enumerate(node.children):
            child_path = f"{current_path} > {child.tag}:nth-child({idx + 1})"
            links.extend(SectionRemovalPlanner.find_dependent_nav_links(child, section_id, child_path))

        if node.shadow_root:
            shadow_path = f"{current_path} > #shadow-root"
            links.extend(SectionRemovalPlanner.find_dependent_nav_links(node.shadow_root, section_id, shadow_path))
    ...
    def plan(self, cids_root: ComponentNode, section_node_path: str) -> RemovalPlan:
        ...
        if not node.removable:
            raise ValueError(f"Node at path {section_node_path} is marked non-removable")
        ...
        for nav_path in affected_nav_entries:
            reflow_actions.append(ReflowAction(
        ...
        elif container_type in (LayoutContainerType.FLEX_ROW, LayoutContainerType.FLEX_COLUMN):
        ...
        if container_type in (LayoutContainerType.STACK, LayoutContainerType.FLEX_COLUMN):
            remaining_count = len(parent.children) - 1
            if remaining_count > 0:
                # Find sibling before and after
                sib_before = parent.children[index - 1] if index > 0 else None
                sib_after = parent.children[index + 1] if index < len(parent.children) - 1 else None

                if sib_before and sib_after:
                    # Sibling before path
                    sib_before_path = f"{parent_path} > {sib_before.tag}:nth-child({index})"
                    ...
                    removed_margin_top = node.styles.get("margin-top", "0px")
                    ...
                    if removed_margin_bottom != "0px" and sib_before.styles.get("margin-bottom", "0px") == "0px":
                        reflow_actions.append(ReflowAction(
        ...
        for sib_path in affected_siblings:
            ...
            if sib_node:
                ...
                if has_index or has_order:
                    if has_index:
                        before_vals["data-section-index"] = sib_node.attributes["data-section-index"]
        ...
        action_order = {"remove_nav_entry": 0, "resize_grid": 1, "recompute_flex_basis": 1, "close_spacing_gap": 2, "renumber_order": 3}
