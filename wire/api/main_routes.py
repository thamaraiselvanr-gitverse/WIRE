...
@router.post("")
async def start_reconstruction(req: ReconstructionRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def get_current_user_file(request: Request, db: AsyncSession):
    ...
    from wire.api.auth import SECRET_KEY, ALGORITHM
...
async def get_project_file(project_id: int, filename: str, request: Request, db: AsyncSession = Depends(get_db)):
    import os
    ...
    # 2. Extract host to resolve path
