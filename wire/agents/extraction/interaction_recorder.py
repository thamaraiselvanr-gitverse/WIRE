...
class InteractionRecorder:
    ...
    def _is_meaningful_diff(before_val: str, after_val: str) -> bool:
        ...
        return InteractionRecorder._normalize_css_value(before_val) != InteractionRecorder._normalize_css_value(after_val)

    async def record_hover_states(self, page: Page, hoverable_elements: list[dict], asset_dir: str) -> list[dict]:
        capped = hoverable_elements[:MAX_INTERACTION_LIMIT]
        logger.info("recording_hover_states", count=len(hoverable_elements), capped_to=len(capped))
        results = []
        ...
        for i, element in enumerate(capped):
            ...
            try:
                ...
                vw, vh = (viewport["width"], viewport["height"]) if viewport else (1920, 1080)
