...
def test_centralized_sanitization_at_cids_level():
    ...
    def find_node(
        node: ComponentNode, tag_name: str, node_id: str = None
    ) -> ComponentNode | None:
    ...
    assert (
        count_nodes(root_node, "script") == 0
    ), "Script tag node was not stripped at CIDS tree level"
    ...
    assert (
        "onclick" not in span_node.attributes
    ), "Event handler attribute was not stripped at CIDS tree level"
    ...
    assert (
        "href" not in link_node.attributes
    ), "Unsafe href protocol was not stripped at CIDS tree level"
...
def test_template_registry_multi_tag_and_ranking():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Register templates
        registry.register(
            "t1", "http://site1.com", ["landing", "ecommerce"], {"rank": 5}
        )
...
def test_token_swap_substitution():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        system.save_tokens(
            "siteA", {"colors": {"primary": "#0000ff", "background": "#000000"}}
        )  # Blue, Black
        system.save_tokens(
            "siteB", {"colors": {"primary": "#ff0000", "background": "#ffffff"}}
        )  # Red, White
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cids_v1 = {
            ...
            "root": {
                "tag": "div",
                "styles": {"color": "red"},
                "children": [{"tag": "span", "text_content": "v1 content"}],
            },
        }
...
def test_preview_csp_sandboxing():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        template_data = {
            "components": [{"id": "c1", "tag": "div", "content": "Preview Item"}]
        }
