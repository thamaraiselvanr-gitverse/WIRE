...
FIXTURE_PATH = os.path.join(os.path.dirname(__file__), "fixtures", "fidelity_target.html")

def _normalize_color(c: str) -> str:
    c = str(c).lower().replace(" ", "")
    if c in ("rgb(255,0,0)", "red"): return "#ff0000"
    if c == "rgb(0,0,0)": return "#000000"
    if c == "rgb(51,51,51)": return "#333333"
    if c == "rgb(204,204,204)": return "#cccccc"
    if c == "rgb(187,187,187)": return "#bbbbbb"
    if c == "rgb(170,170,170)": return "#aaaaaa"
    if c == "rgb(68,68,68)": return "#444444"
    if c == "rgb(255,255,255)": return "#ffffff"
    return c
...
def parsed_cids():
    ...
    cids_root = HTMLToCidsParser.parse(soup, style_map)
    # The current parser starts from <body> by default, we need it to evaluate <html> 
    # to fetch --brand-color. So we explicitly run _process_node on html_node:
...
async def test_fidelity_parity(parsed_cids):
    ...
    assert _normalize_color(span_node.styles.get("color", "")) == "#ff0000", f"Inheritance failed. Expected #ff0000, got {span_node.styles.get('color')}"

    # Test 2: Specificity ID > Class
    def find_div_special(n):
        if n.tag == "div" and n.attributes.get("id") == "special-box": return n
        for c in n.children:
    ...
    def find_li(n):
        if n.tag == "li" and n.attributes.get("id") == "list-1": return n
        for c in n.children:
            res = find_li(c)
            if res: return res
        return None
