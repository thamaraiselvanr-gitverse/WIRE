...
from wire.schema.canonical import ComponentNode, CanonicalDesignSchema, DesignTokens
from wire.schema.layout_schema import LayoutContainerType, ReflowAction, RemovalPlan
...
def _make_node(tag: str, attrs: dict = None, styles: dict = None, children: list = None, slot_id: str = None, shadow_root: ComponentNode = None, removable: bool = True, text: str = None) -> ComponentNode:
...
class TestRemovalPlanning:
    def test_basic_removal_plan(self):
        ...
        root = _make_node("div", styles={"display": "block"}, children=[
            _make_node("nav", {"id": "main-nav"}, children=[
                _make_node("a", {"href": "#hero"}),
                _make_node("a", {"href": "#about"}),
                _make_node("a", {"href": "#contact"}),
            ]),
            _make_node("section", {"id": "hero", "data-section-index": "1"}, styles={"margin-top": "20px", "margin-bottom": "20px"}),
            _make_node("section", {"id": "about", "data-section-index": "2"}, styles={"margin-top": "20px"}, removable=False),
            _make_node("section", {"id": "contact", "data-section-index": "3"}, styles={"margin-top": "20px"}),
        ])
        ...
        with pytest.raises(ValueError, match="Cannot target nodes inside a shadow root"):
...
class TestGridReflow:
    def test_grid_resizing(self):
        ...
        grid_parent = _make_node("div", styles={"display": "grid", "grid-template-columns": "repeat(3, 1fr)"}, children=[
...
class TestFlexReflow:
    def test_flex_redistribution(self):
        ...
        flex_parent = _make_node("div", styles={"display": "flex", "flex-direction": "row"}, children=[
        ...
        recompute_actions = [a for a in plan.reflow_actions if a.action_type == "recompute_flex_basis"]
...
class TestSpacingScale:
    def test_close_spacing_gap(self):
        ...
        root = _make_node("div", styles={"display": "block"}, children=[
            ...
            _make_node("section", {"id": "sec2"}, styles={"margin-top": "24px", "margin-bottom": "32px"}),
        ...
        spacing_actions = [a for a in plan.reflow_actions if a.action_type == "close_spacing_gap"]
...
class TestIntegrityValidator:
    ...
    def trees(self):
        ...
        orig = _make_node("div", children=[
            _make_node("nav", children=[_make_node("a", {"href": "#hero"})]),
            _make_node("section", {"id": "hero", "data-section-index": "1"}, styles={"margin-bottom": "24px"}, slot_id="slot_hero"),
            _make_node("section", {"id": "about", "data-section-index": "2"}, styles={"margin-bottom": "24px"}, slot_id="slot_about"),
        ])
        
        # Correctly reflowed mutated tree (removed section 1 "hero" and its nav link, renumbered order)
        mutated_valid = _make_node("div", children=[
            _make_node("nav", children=[]),
            _make_node("section", {"id": "about", "data-section-index": "1"}, styles={"margin-bottom": "24px"}, slot_id="slot_about"),
        ])
    ...
    def test_orphaned_nav_violation(self, trees):
        ...
        mutated_invalid = _make_node("div", children=[
            _make_node("nav", children=[_make_node("a", {"href": "#hero"})]),
            _make_node("section", {"id": "about", "data-section-index": "1"}, styles={"margin-bottom": "24px"}),
        ])
        validator = StructuralIntegrityValidator()
        report = validator.validate(orig, mutated_invalid, "root > section:nth-child(2)")
    ...
    def test_empty_grid_cells_violation(self, trees):
        ...
        mutated_invalid = _make_node("div", styles={"display": "grid", "grid-template-columns": "repeat(3, 1fr)"}, children=[])
        # Force dummy section path to avoid lookups
        orig_dummy = _make_node("div", children=[_make_node("div", {"id": "removed_sec"})])
        validator = StructuralIntegrityValidator()
        report = validator.validate(orig_dummy, mutated_invalid, "root > div:nth-child(1)")
        assert report.passed is False
    ...
    def test_contiguous_ordering_violation(self, trees):
        ...
        orig_dummy = _make_node("div", children=[_make_node("section", {"id": "removed_sec"})])
        validator = StructuralIntegrityValidator()
        report = validator.validate(orig_dummy, mutated_invalid, "root > section:nth-child(1)")
        assert report.passed is False
    ...
    def test_spacing_scale_violation(self, trees):
        ...
        mutated_invalid = _make_node("div", children=[
            _make_node("section", {"id": "about"}, styles={"margin-bottom": "999px"}),
        ])
        validator = StructuralIntegrityValidator()
        report = validator.validate(orig, mutated_invalid, "root > section:nth-child(2)")
        assert report.passed is False
    ...
    def test_dangling_slot_id_violation(self, trees):
        ...
        mutated_invalid = _make_node("div", children=[
            _make_node("section", {"id": "about", "data-section-index": "1"}, styles={"margin-bottom": "24px"}, slot_id="slot_hero"),
        ])
        validator = StructuralIntegrityValidator()
        report = validator.validate(orig, mutated_invalid, "root > section:nth-child(2)")
        assert report.passed is False
...
class TestShadowDOMBoundary:
    def test_shadow_dom_atomic_boundaries(self):
        ...
        shadow_content = _make_node("div", children=[
            _make_node("h1", text="Inside Shadow"),
        ])
        shadow_host = _make_node("my-widget", {"id": "host"}, shadow_root=shadow_content)
        ...
        mutated = engine.execute(root, plan)
        assert len(mutated.children) == 2
        assert mutated.children[0].tag == "my-widget" # host correctly repositioned to index 0
        assert mutated.children[0].shadow_root is not None
        assert mutated.children[0].shadow_root.children[0].tag == "h1" # interior untouched
...
class TestEndToEndRemovalOrchestration:
    def test_remove_sections_pipeline(self):
        ...
        with tempfile.TemporaryDirectory() as base_dir:
            ...
            cids_root = _make_node("div", children=[
                _make_node("nav", children=[_make_node("a", {"href": "#hero"})]),
                _make_node("section", {"id": "hero", "data-section-index": "1"}, styles={"margin-bottom": "24px"}, slot_id="slot_hero"),
                _make_node("section", {"id": "about", "data-section-index": "2"}, styles={"margin-bottom": "24px"}, slot_id="slot_about"),
            ])
            ...
            assert result.mutated_root.children[1].attributes["data-section-index"] == "1"
            ...
            with open(os.path.join(run_dir, "output_react.jsx"), "r", encoding="utf-8") as f:
                react_code = f.read()
