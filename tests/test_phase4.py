...
async def test_wait_for_dom_stability():
    ...
    try:
        ...
        async def add_dynamic_nodes():
            ...
            await page.evaluate(
                "() => { document.body.innerHTML += '<div>Dynamic 1</div>'; }"
            )
            await asyncio.sleep(0.5)
            await page.evaluate(
                "() => { document.body.innerHTML += '<div>Dynamic 2</div>'; }"
            )
        ...
        await session.wait_for_dom_stability(
            page, timeout_ms=3000, check_interval_ms=200
        )
        await task
...
async def test_shadow_dom_piercer_and_cids_mapping(shadow_fixture_url):
    ...
    try:
        ...
        nested_element_node = next(
            c for c in div_node["children"] if c["tag"] == "nested-element"
        )
        assert nested_element_node["shadow_root"] is not None
        assert (
            nested_element_node["shadow_root"]["style_provenance"] == "cascade_resolved"
        )

        # Verify adoptedStyleSheets mapping
        nested_p = next(
            c for c in nested_element_node["shadow_root"]["children"] if c["tag"] == "p"
        )
        ...
        def dict_to_node(d: dict) -> ComponentNode:
            ...
            shadow_root = (
                dict_to_node(d["shadow_root"]) if d.get("shadow_root") else None
            )
        ...
        real_root = HTMLToCidsParser.parse(
            soup, style_map={}, interactions_map={}, shadow_roots_map=shadow_roots_map
        )
        ...
        real_root_styled = HTMLToCidsParser.parse(
            soup,
...
def test_input_blueprint_validation():
    ...
    assert any(
        f["slot_id"] == "required_title" and "Type mismatch" in f["message"]
...
def test_knowledge_index_enhanced_queries():
    ...
    res_color = index.query(
        color_similarity_target="#ff0505", color_similarity_threshold=20.0
    )
...
async def test_mixed_provenance_boundary(shadow_fixture_url):
    ...
    try:
        ...
        assert (
            "background-color" not in closed_div.styles
            or closed_div.styles.get("background-color") != "rgb(0, 255, 0)"
        )
    finally:
        await page.close()
        await session.stop()
...
async def test_shadow_dom_visual_fidelity(shadow_fixture_url):
    ...
    try:
        ...
        cids = CanonicalDesignSchema(
            url=shadow_fixture_url, tokens=DesignTokens(), root=real_root
        )
        ...
        assert (
            similarity >= 95.0
        ), f"Open Shadow DOM visual fidelity ({similarity}%) is below 95% threshold"

        # Verify host2 (closed shadow root, computed fallback)
        result_host2 = diff_engine.compare_screenshots(
            orig_img_path_host2, recon_img_path_host2
        )
        similarity_host2 = result_host2["similarity_percent"]
        print(f"\n--- CLOSED SHADOW DOM VISUAL FIDELITY: {similarity_host2}% ---")
        assert (
            similarity_host2 >= 95.0
        ), f"Closed Shadow DOM visual fidelity ({similarity_host2}%) is below 95% threshold"
